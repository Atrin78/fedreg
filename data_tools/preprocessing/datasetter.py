...
def data_distributer(
    ...
    for client_idx, dataidxs in net_dataidx_map.items():
        ...
        local_loaders[client_idx]["train_for_cka"] = DATA_LOADERS[dataset_name](
            root, train=True, shuffle = False, drop_last=True, batch_size=int(batch_size/5), dataidxs=dataidxs,
        )
    ...
    if net_dataidx_map_test is not None:
        for client_idx, dataidxs in net_dataidx_map_test.items():
            local_testloader = DATA_LOADERS[dataset_name](
                root, train=False, shuffle = False, batch_size=batch_size, dataidxs=dataidxs,
            )
            local_loaders[client_idx]["test"] = local_testloader
            local_loaders[client_idx]["test_datasize"] = len(dataidxs)
    ...
    global_loaders = {
        ...
        "train_for_cka": DATA_LOADERS[dataset_name](root, train=True,shuffle = False, drop_last=True,batch_size=int(batch_size/5)),
    ...
    global_loaders["test_datasize"] = len(global_loaders["test"].dataset)

...
#                     loss += 0.01 * loss_r_feature
    ...
    #         hook.close()
        ...
        original_dataset_lost = [(original_dataset.squeeze(1)[i, :, :, :] * 255).round().to(torch.int) for i in range(20)]
        gen_dataset_lost = [(gen_dataset.squeeze(1)[i, :, :, :] * 255).round().to(torch.int)  for i in range(20)]

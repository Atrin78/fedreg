...
def step_func(model, data, fed, active_layer):
    ...
    median_parameters = []
    for p in median_model.net.parameters():
        p.requires_grad = False
    if median_model.bottleneck != None:
        for p in median_model.bottleneck.parameters():
            p.requires_grad = False
    for p in median_model.head.parameters():
        p.requires_grad = False
    for name, layer in median_model.named_modules():
        if name in active_layer:
            median_parameters += list(layer.parameters())
            for p in layer.parameters():
                p.requires_grad = True

    old_parameters = []
    for p in old_model.net.parameters():
        p.requires_grad = False
    if old_model.bottleneck != None:
        for p in old_model.bottleneck.parameters():
            p.requires_grad = False
    for p in old_model.head.parameters():
        p.requires_grad = False
    for name, layer in old_model.named_modules():
        if name in active_layer:
            old_parameters += list(layer.parameters())
            for p in layer.parameters():
                p.requires_grad = True

    penal_parameters = []
    for p in penal_model.net.parameters():
        p.requires_grad = False
    if penal_model.bottleneck != None:
        for p in penal_model.bottleneck.parameters():
            p.requires_grad = False
    for p in penal_model.head.parameters():
        p.requires_grad = False
    for name, layer in penal_model.named_modules():
        if name in active_layer:
            penal_parameters += list(layer.parameters())
            for p in layer.parameters():
                p.requires_grad = True

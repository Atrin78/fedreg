    ...
    def train(self):
        ...
        for r in range(self.start_round+1,self.num_rounds):
            ...
            for idx, c in enumerate(active_clients):
                ...
                soln, stats = c.solve_inner(num_epochs=self.num_epochs, step_func=partial(step_func, global_model = global_model, active_layer = self.active_layers))  # stats has (byte w, comp, byte r)
                soln = [1.0, soln[1]]
                w += soln[0]
                if len(csolns) == 0:
                    for x in soln[1]:

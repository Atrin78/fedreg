...
def prepare_data(data_size, datasets, public_dataset, im_size):
    ...
    VHL_virtualset = Modified_medmnist(data_path="../../Federated-Learning/data/VHL/", split='VHL', chunk='0')
    VHL_generateset = Modified_medmnist(data_path="../../Federated-Learning/data/VHL/", split='VHL', chunk='0')
    ...
    if public_dataset != None:
        if public_dataset == 'drishti':
            generatsets.append(CustomDataset(ImageFolder(drishti_train_path, transform=transform_tensor), data_size, transform=transform_drishti2))
            virtualsets.append(CustomDataset(ImageFolder(drishti_train_path, transform=transform_tensor), data_size, transform=transform_drishti2))

        elif public_dataset == 'kaggle':
            generatsets.append(CustomDataset(ImageFolder(kaggle_train_path, transform=transform_tensor), data_size, transform=transform_kaggle2))
            virtualsets.append(CustomDataset(ImageFolder(kaggle_train_path, transform=transform_tensor), data_size, transform=transform_kaggle2))

        elif public_dataset == 'rim':
            generatsets.append(CustomDataset(ImageFolder(rim_train_path, transform=transform_tensor), data_size, transform=transform_rim2))
            virtualsets.append(CustomDataset(ImageFolder(rim_train_path, transform=transform_tensor), data_size, transform=transform_rim2))

        elif public_dataset == 'refuge':
            generatsets.append(CustomDataset(ImageFolder(refuge_train_path, transform=transform_tensor), data_size, transform=transform_refuge2))
            virtualsets.append(CustomDataset(ImageFolder(refuge_train_path, transform=transform_tensor), data_size, transform=transform_refuge2))
    ...
    test_data['drishti'] = {'x':x.cpu().detach().numpy(), 'y':y.cpu().detach().numpy()}
    x, y = next(iter(DataLoader(kaggle_testset, batch_size=len(kaggle_testset))))

...
transform_fun = transforms.Compose(
            [
             ...
             transforms.ToTensor(),
             transforms.Lambda(lambda x: torch.stack([torch.unsqueeze(torch.squeeze(x, 1), -1),torch.unsqueeze(torch.squeeze(x, 1), -1),torch.unsqueeze(torch.squeeze(x, 1), -1)],-1)/3.0)
